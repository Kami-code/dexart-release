...
from models.pointnet import PointNet, EncoderDecoder, ChamLoss
...
class Solver(object):
    ...
    def train(self):
        for epoch in range(self.config['num_epochs']):
            ...
            for i, (points, _) in tqdm(enumerate(self.train_loader)):  # we do not need labels
                ...
                coarse, fine = self.model(points)
                loss = self.criterion(coarse, fine, points, 0.3)
                loss.backward()
                self.optimizer.step()
    ...
    def save(self, epoch):
        torch.save(self.model.state_dict(), f"{self.config['log_dir']}/complete_{epoch}.pth")
        torch.save(self.model.pointnet.state_dict(), f"{self.config['log_dir']}/complete_pn_{epoch}.pth")
...
def main():
    ...
    config = {
        ...
        'log_step': 10,
        'val_step': 1,
        'log_dir': f'log/reconstruction/{cat}/{run}',
        'lr': 1e-3,
        'classes': 4,
        'save_step': 10,
        'cat': cat,
    ...
    if args.eval is not None:
        solver.load(f'log/reconstruction/{cat}/{run}/{args.eval}')
        pass
    elif args.vis is not None:
        solver.load(f'log/reconstruction/{cat}/{run}/{args.vis}')
